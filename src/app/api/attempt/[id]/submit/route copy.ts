  ...
  try {
    ...
    for (const question of attempt.quiz.questions) {
      ...
      if (
        ...
        console.log(`--- Validasi Q: ${question.id} ---`);
        console.log(`Jawaban User (ID): ${userAnswerValue}`);
        console.log(`Jawaban Benar di DB (ID): ${correctOption?.id}`);
        console.log("----------------------------");
        ...
        // Logic validasi Short Answer perlu disesuaikan (misal: membandingkan dengan jawaban di DB)
        // Untuk demo, kita anggap jawaban benar ada di `explanation` (TIDAK DISARANKAN untuk produksi)
        if (
          question.explanation &&
          userAnswerValue?.toLowerCase().trim() ===
            question.explanation.toLowerCase().trim()
        ) {
          isCorrect = true;
        }
        userAnswerRecords.push({
          questionId: question.id,
          shortAnswer: userAnswerValue,
          isCorrect: isCorrect,
          attemptId: attemptId,
        });
      }
    ...
    await prisma.$transaction(async (tx) => {
      // 1. Gunakan 'upsert' untuk setiap jawaban
      ...
      // 3. Update jumlah pengerjaan kuis
