...
export default function TakeQuizPage() {
  ...
  useEffect(() => {
    const loadQuiz = async () => {
      try {
        setStatus("loading");
        // API response sekarang bisa berisi inProgressAttempt
        const data = await fetchQuizForTaking(quizId);
        setQuizData(data);
        setTimeLeft(data.duration * 60);

        // [MODIFIKASI] Jika ada attempt yang sedang berjalan, pulihkan state
        if (data.inProgressAttempt) {
          const totalDurationInSeconds = data.duration * 60;
          const startTime = new Date(
            data.inProgressAttempt.startedAt
          ).getTime();
          const now = new Date().getTime();
          const elapsedSeconds = Math.floor((now - startTime) / 1000);
          const remainingSeconds = totalDurationInSeconds - elapsedSeconds;
          setTimeLeft(remainingSeconds > 0 ? remainingSeconds : 0);
          setAttemptId(data.inProgressAttempt.attemptId);
          setAnswers(data.inProgressAttempt.savedAnswers);
          setCurrentView("quiz"); // Langsung masuk ke mode pengerjaan kuis
        }
  ...
  const handleAnswer = async (questionId: string, answerValue: string) => {
    ...
    // Jika attempt belum dimulai, jangan lakukan apa-apa
    if (!attemptId) return;
    ...
    try {
      ...
      // Anda bisa menambahkan feedback kecil di sini jika perlu
    } catch (error) {
      console.error("Gagal menyimpan jawaban:", error);
      // Mungkin tampilkan notifikasi toast "gagal menyimpan"
    }
  };
