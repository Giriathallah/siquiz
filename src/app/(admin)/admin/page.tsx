...
// ... (StatCard, DifficultyBadge, StatusBadge components tetap sama)
function StatCard({
  title,
  value,
  icon: Icon,
...
export default function AdminDashboard() {
  const [stats, setStats] = useState<DashboardStats | null>(null); // Explicit type to avoid 'never'
  const [recentQuizzes, setRecentQuizzes] = useState([]); // Use empty array
    ...
    async function fetchData() {
      try {
        ...
        const [statsRes, quizzesRes, attemptsRes] = await Promise.all([
          ...
          fetch("/api/admin/recent-quizzes"),
          fetch("/api/admin/recent-attempts"),
        ]);

        if (!statsRes.ok || !quizzesRes.ok || !attemptsRes.ok) {
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <div className="text-xs text-text-subtle">
                        {(stats?.totalUsers || 0).toLocaleString()} users
                      </div>
                    </div>
                  </div>
                </Button>
