...
import { signIn, oAuthSignIn } from "@/actions/auth"; // [MODIFIED] Import server actions
...
// [NEW] Reusable Alert Component for displaying server feedback
const FormAlert: FC<{ message?: string }> = ({ message }) => {
  if (!message) return null;

  return (
    <div className="flex items-center gap-3 p-3 rounded-lg border bg-red-500/10 border-red-500/50 text-red-400">
...
const SignInPage: FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    startTransition(async () => {
      ...
      // On success, the server action will redirect the user automatically.
      // No client-side navigation is needed.
    });
  };
              ...
              <div className="flex items-center justify-center gap-4 mb-6">
                {oauthProviders.map((p) => (
                  <OAuthButton
                    ...
                    isPending={isPending}
                    hoverColorClass={p.hoverColorClass}
                  />
                ))}
              </div>
